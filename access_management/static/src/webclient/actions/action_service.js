...
export function makeActionManager(env, router = _router) {
     ...
     */
    async function _getActionParams(state) {
        const options = {};
        ...
        if (!actionRequest) {
            ...
            if (actionStack?.length > 1) {
                ...
                const params = await _getActionParams(nextState);
        ...
        if (actionRequest && actionRequest.res_model) {
            const result = await rpc('/check_restricted_views', { model: actionRequest.res_model })
            if (actionRequest.views) {
                actionRequest.views = actionRequest.views.filter(view => !result.includes(view[1]))
            }
        }
    ...
    async function restore(jsId) {
        ...
        if (controller.virtual) {
            const actionParams = await _getActionParams(controller.state);
            if (!actionParams) {
    ...
    async function loadState(state = router.current) {
        ...
        const actionParams = await _getActionParams(state);
        if (actionParams) {
